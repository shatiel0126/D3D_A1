...
{
	//GeometryGenerator is a utility class for generating simple geometric shapes like grids, sphere, cone, and boxes
	...
	//Createcone(float bottomRadius, float topRadius, float height, uint32 sliceCount, uint32 stackCount)
	...
	UINT coneVertexOffset = 0;


	// Cache the starting index for each object in the concatenated index buffer.
	UINT coneIndexOffset = 0;
	...
	auto totalVertexCount = cone.Vertices.size();
...
void ShapesApp::BuildRenderItems()
{
	...
	XMStoreFloat4x4(&coneRitem->World, XMMatrixScaling(2.0f, 2.0f, 2.0f) * XMMatrixTranslation(0.0f, 0.5f, 0.0f));
	coneRitem->ObjCBIndex = 0;
	coneRitem->Geo = mGeometries["shapeGeo"].get();
	coneRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	coneRitem->IndexCount = coneRitem->Geo->DrawArgs["cone"].IndexCount;  //2520
	coneRitem->StartIndexLocation = coneRitem->Geo->DrawArgs["cone"].StartIndexLocation; //0
	coneRitem->BaseVertexLocation = coneRitem->Geo->DrawArgs["cone"].BaseVertexLocation; //0
